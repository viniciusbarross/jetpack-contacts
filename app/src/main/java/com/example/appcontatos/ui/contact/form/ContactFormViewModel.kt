...
import com.example.appcontatos.ui.Arguments
...
class ContactFormViewModel(
    ...
    var uiState: ContactFormState by mutableStateOf(ContactFormState(contactId = contactId))
        ...
        viewModelScope.launch {
            ...
            uiState = if(contact == null){
                ...
                uiState.copy(
                    isLoading = false,
                    contact = contact,
                    firstName = uiState.firstName.copy(value = contact.firstName),
                    lastName = uiState.lastName.copy(value = contact.lastName),
                    phone = uiState.phone.copy(value = contact.phoneNumber),
                    email = uiState.email.copy(value = contact.email),
                    isFavorite = uiState.isFavorite.copy(value = contact.isFavorite),
                    birthDate = uiState.birthDate.copy(value = contact.birthDate),
                    type = uiState.type.copy(value = contact.type)
                )
            }
        }
    }
